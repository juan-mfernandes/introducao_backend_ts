...
function getById(req: Request, res: Response) {
  const id = Number(req.params.id);
  const taskFound = tasksService.getTaskById(id);
...
function doneTask(req: Request, res: Response) {
    const id = Number(req.params.id);
    const foundTask = tasksService.getTaskById(id);

    if(!foundTask) {
        return res.status(404).json({ error: "Tarefa não encontrada." });
    }

    const taskChanged = tasksService.doneTask(foundTask.id);
    if(!taskChanged) {
        return res.status(400).json({error: "Dados inválidos."});
    }

    return res.status(202).json({success: "Tarefa concluída: ", taskChanged});
};
...
export default { show, getById, doneTask, create, update, del };